...
- **Importação de dados**: Como carregar dados a partir de arquivos CSV utilizando o `pandas.read_csv()`.
- **Limpeza e tratamento de dados**: Identificação e tratamento de valores missing, duplicates e transformação de tipos de dados.
- **Análise exploratória**: Uso de métodos como `describe()`, `value_counts()`, `corr()` e agrupamentos com `groupby()` para entender a estrutura e distribuição dos dados.
